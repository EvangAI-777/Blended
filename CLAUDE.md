...
| Any startup — build 106 | `BKE_libblock_alloc_in_lib` crashed with null pointer dereference. Root cause: `BKE_libblock_runtime_ensure(*id)` was called at line 1377 to initialize runtime data, but `id` was dereferenced BEFORE the null check at line 1379. If `BKE_libblock_alloc_notest(type)` returned nullptr, this immediately crashed at `EXCEPTION_ACCESS_VIOLATION 0x00007FF6dCAFAD54`. Trigger path: startup file loading → `BKE_scene_add("Empty")` in `blendfile.cc:1054` to create default scene → `BKE_id_new<Scene>` → `BKE_libblock_alloc_in_lib` → null dereference. Fix: moved `BKE_libblock_runtime_ensure(*id)` inside the `if (id)` null check. | Various (ID allocation infrastructure) | PR #240 commit `ddea9c75` |
